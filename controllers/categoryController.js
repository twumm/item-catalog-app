...
exports.index = function(req, res, next) {
    async.parallel({
            ...
            res.render('index', { title: 'Item Catalog Home', error: err, data: results });
        })
...
exports.category_detail = function(req, res, next) {
    async.parallel({
        ...
        // Successful, so render
        res.render('category_detail', { title: 'Category detail', category: results.category, items: results.item });
    });
};

// Display Category create form on GET.
exports.category_create_get = function(req, res, next) {
    res.render('category_form', { title: 'Create category', userid: req.session.userId });
    ...
    Category.findById(req.params.id, 'title')
        ...
        .exec(function(err, category) {
            if (err) {
                ...
                res.render('category_delete', { title: 'Delete category', category: category });
            }
        })
...
exports.category_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (results.items.length > 0) { // If there is at least one item in the category
            ...
            res.render('category_delete', { title: 'Delete category', category: results.category, items: results.items, error: 'You are not the owner.' })
        }
...
exports.category_update_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) {
            ...
            res.render('category_detail', { title: 'Category detail', category: results.category, items: results.item, error: 'You do not own this category.' })
        } else {
            res.render('category_form', { title: 'Edit Category', category: results.category, users: results.user });
        }
    });
